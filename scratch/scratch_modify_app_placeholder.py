...
new_section_code = """elif page == "✏️ Редактирование нагрузки":
    ...
    if not unique_teachers:
        ...
        if not sapr_loads:
            ...
            # Считываем заглушки по умолчанию из шаблона
            placeholders = {}
            if template_path and os.path.exists(template_path):
                try:
                    ...
                    for key, coord_info in STRICT_PLAN_COORDINATES.items():
                        ...
                        placeholders[key] = str(val).strip() if val is not None else ""
                except Exception:
                    pass
            ...
            metadata_labels = {
                ...
                "employee_contract_num": "Договор (дата, номер)",
                "employee_contract_duration": "Срок действия договора",
                "employee_edu_institution": "Образовательная организация",
                "employee_edu_specialty": "Направление подготовки",
            ...
            db_values = {
                ...
                "employee_contract_num": contract_number + (" от " + contract_date if contract_date else ""),
                "employee_contract_duration": contract_duration,
                "employee_edu_institution": edu_institution + (" " + edu_year if edu_year else ""),
                "employee_edu_specialty": edu_specialty,
